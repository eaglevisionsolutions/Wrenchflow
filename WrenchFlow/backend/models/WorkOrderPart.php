...
class WorkOrderPart {
    ...
    public function getPartsByWorkOrder($shop_id, $work_order_id) {
        $query = "SELECT p.name AS part_name, p.part_number, wop.quantity_used, wop.volume_used 
                  FROM work_order_parts wop
                  INNER JOIN parts p ON wop.part_id = p.id
                  WHERE wop.shop_id = :shop_id AND wop.work_order_id = :work_order_id";
        $stmt = $this->db->prepare($query);
        $stmt->bindParam(':shop_id', $shop_id);
        $stmt->bindParam(':work_order_id', $work_order_id);
        $stmt->execute();
